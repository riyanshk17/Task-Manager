...
import { Layers, Users, LogOut, Copy, Check, Plus, ChevronDown, Sun, Moon, UserMinus, MessageSquare, Settings, ShieldCheck, Menu, X } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className={`mobile-nav-drawer ${mobileMenuOpen ? 'open' : ''}`}>
        {/* User Card */}
        <div className="user-pill" onClick={() => { onOpenSettings(); setMobileMenuOpen(false); }} style={{ cursor: 'pointer', width: '100%', justifyContent: 'flex-start' }}>
          {user?.avatarUrl ? (
            <img 
              src={user.avatarUrl} 
              alt={user.name} 
          ...
          <div className="user-info" style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <span className="role-badge manager" style={{ fontSize: '0.6rem' }}>
        ...
        {userTeams.length > 0 && (
          <div style={{ width: '100%' }}>
            ...
            <select 
              className="select-filter"
              style={{ width: '100%', fontWeight: 600, color: 'var(--primary)', background: 'rgba(99, 102, 241, 0.12)' }}
              value={activeTeam?.id || ''}
              onChange={e => {
        ...
        {/* Team Code & Quick Actions */}
        {activeTeam && (
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            <div className="team-badge-pill" style={{ flex: 1, justifyContent: 'center' }} onClick={() => { onOpenTeamModal(); setMobileMenuOpen(false); }}>
              <Users size={15} />
              <span style={{ fontFamily: 'monospace', fontWeight: 700 }}>Code: {activeTeam.code}</span>
              <button onClick={handleCopyCode} style={{ background: 'none', border: 'none', color: 'inherit', cursor: 'pointer' }}>
                {copied ? <Check size={14} className="text-emerald-400" /> : <Copy size={14} />}
              </button>
            </div>
            {onLeaveTeam && (
        ...
        {isDeveloper && onOpenDeveloperControl && (
          <button 
            onClick={() => { setMobileMenuOpen(false); onOpenDeveloperControl(); }}
            className="btn btn-primary btn-sm"
            style={{ width: '100%', background: 'rgba(99, 102, 241, 0.2)', borderColor: 'rgba(99, 102, 241, 0.5)', color: '#a5b4fc', fontWeight: 700 }}
          >
            <ShieldCheck size={16} /> Dev Controls
          </button>
        )}
